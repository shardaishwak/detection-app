...
export default function App() {
  const camera = useRef();
  const imgLeft = useRef(new Animated.Value(20)).current;
  const imgToggleRight = useRef(new Animated.Value(0)).current;
  ...
  useEffect(() => {
    if (!showImg) {
      ...
      setTimeout(() => {
        Animated.timing(imgToggleRight, {
          toValue: -24,
          duration: 250,
          useNativeDriver: false,
        }).start();
  ...
  return (
    <View style={styles.container}>
      {!imageUri && (
        <>
          <Detector imageUri={chosenImage} similarityScoreRef={similarity} />
          {/* <Component
						ref={camera}
						autorender
          ...
          <View style={styles.topContainer}>
            ...
            {(chosenImage && (
              <Animated.View style={[styles.imageContainer, { left: imgLeft }]}>
                <Pressable
